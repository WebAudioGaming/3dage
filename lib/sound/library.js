...
module.exports = function (libraryDefinition, options) {
  ...
  return {
    ...
    '_': {
      parseSoundName: parseSoundName,
      validateLibraryDefinition: validateLibraryDefinition,
      preloadingProgressFromSources: preloadingProgressFromSources
    }
  }
}
...
function preloadingProgressFromSources (sources) {
  var count = sources.length

  return Rx.Observable.from(sources)
    .flatMap(function (soundInstance) {
      return Rx.Observable.fromEvent(soundInstance, 'load')
    })
...
function preloadingProgressStream (sounds) {
  ...
  return preloadingProgressFromSources(sources)
}
